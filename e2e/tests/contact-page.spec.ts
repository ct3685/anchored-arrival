...
test.describe('Contact Page — Entry Screen', () => {
  ...
  test('renders the entry screen with title and stamps', async ({ page }) => {
    await expect(page.getByText('OPERATION: CONTACT MORGIE')).toBeVisible({ timeout: 10000 });
    await expect(page.getByText('TOP SECRET')).toBeVisible();
    await expect(page.getByText('CLASSIFIED', { exact: true })).toBeVisible();
    await expect(page.getByText('This dossier is classified')).toBeVisible();
  });

  test('shows ACCEPT MISSION button after delay', async ({ page }) => {
    const acceptBtn = page.getByRole('button', { name: /ACCEPT MISSION/i });
    await expect(acceptBtn).toBeVisible({ timeout: 5000 });
  });

  test('shows Skip the Mission link after delay', async ({ page }) => {
    const skipBtn = page.getByRole('button', { name: /Skip the Mission/i });
    await expect(skipBtn).toBeVisible({ timeout: 5000 });
  });

  test('navbar includes Contact link', async ({ page }) => {
...
test.describe('Contact Page — Cipher Puzzle (Phase 1)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByText('PHASE 1: DECODE THE CIPHER')).toBeVisible({ timeout: 5000 });
  });

  test('renders cipher puzzle with slider and encrypted text', async ({ page }) => {
    ...
    // Use the slider inside the card (not the mini player slider)
    await expect(page.locator('.MuiSlider-sizeMedium')).toBeVisible();
  });

  test('shows alphabet mapping', async ({ page }) => {
    await expect(page.getByText(/Original: ABCDEFGHIJKLMNOPQRSTUVWXYZ/)).toBeVisible();
    await expect(page.getByText(/Rotated:/)).toBeVisible();
  });

  test('solving cipher (shift=3) shows success and unlocks contact', async ({ page }) => {
    const slider = page.locator('.MuiSlider-sizeMedium');
    const box = await slider.boundingBox();
    if (!box) throw new Error('Slider not found');
    // Position 3 out of 25 on the slider
    await page.mouse.click(box.x + (box.width * 3 / 25), box.y + box.height / 2);

    await expect(page.getByText('AGENT MORGIE IS THE MVP')).toBeVisible({ timeout: 3000 });
    await expect(page.getByText('PHASE 1 COMPLETE')).toBeVisible();
    await expect(page.getByText('morgie@agentmorgie.com').first()).toBeVisible();
    await expect(page.getByRole('button', { name: /PROCEED TO PHASE 2/i })).toBeVisible();
  });
  ...
  test('incorrect shift does not show success', async ({ page }) => {
    ...
    const box = await slider.boundingBox();
    if (!box) throw new Error('Slider not found');
    await page.mouse.click(box.x + (box.width * 5 / 25), box.y + box.height / 2);
...
test.describe('Contact Page — Trivia Puzzle (Phase 2)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByText('PHASE 2: AGENT TRIVIA')).toBeVisible({ timeout: 5000 });
  });

  test('renders all three trivia questions', async ({ page }) => {
    await expect(page.getByText("What platform is Agent Morgie's primary HQ?")).toBeVisible();
    await expect(page.getByText("What's Agent Morgie's creator motto?")).toBeVisible();
    await expect(page.getByText("Agent Morgie's community is built on...")).toBeVisible();
  });

  test('wrong answer shakes and stays clickable', async ({ page }) => {
    await page.getByRole('button', { name: 'YouTube' }).click();
    // Correct answer should still be enabled
  ...
  test('correct answers complete phase 2', async ({ page }) => {
    ...
    await expect(page.getByText('PHASE 2 COMPLETE')).toBeVisible({ timeout: 3000 });
    await expect(page.getByText(/Secure Mail Drop/i)).toBeVisible();
    await expect(page.getByRole('button', { name: /PROCEED TO PHASE 3/i })).toBeVisible();
  });
});
...
test.describe('Contact Page — Safe Puzzle (Phase 3)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByText('PHASE 3: CRACK THE SAFE')).toBeVisible({ timeout: 5000 });
  });

  test('renders safe with four dials', async ({ page }) => {
    await expect(page.getByText('Combination Lock')).toBeVisible();
    for (let i = 1; i <= 4; i++) {
      await expect(page.getByRole('button', { name: `Increase digit ${i}` })).toBeVisible();
      await expect(page.getByRole('button', { name: `Decrease digit ${i}` })).toBeVisible();
    }
  ...
  test('shows hint after 3 wrong attempts', async ({ page }) => {
    ...
    await expect(page.getByText(/Need another hint/i)).toBeVisible({ timeout: 2000 });
  });

  test('entering 1-0-0-0 solves the safe', async ({ page }) => {
    await page.getByRole('button', { name: 'Increase digit 1' }).click();
    await expect(page.getByText('SAFE CRACKED! DOSSIER UNLOCKED!')).toBeVisible({ timeout: 3000 });
  });
});

test.describe('Contact Page — Dossier (Earned)', () => {
  test('solving all puzzles reveals full dossier with self-destruct', async ({ page }) => {
    await navigateToSafeSolved(page);
    await expect(page.getByText('CLASSIFIED DOSSIER')).toBeVisible({ timeout: 10000 });
    await expect(page.getByText('morgie@agentmorgie.com').first()).toBeVisible({ timeout: 10000 });
  ...
  test('DISARM button stops the self-destruct', async ({ page }) => {
    ...
    await expect(page.getByText(/SELF-DESTRUCT:/i)).not.toBeVisible({ timeout: 2000 });
    await expect(page.getByText('CLASSIFIED DOSSIER')).toBeVisible();
  });

  test('dossier shows all contact info and social links', async ({ page }) => {
    await navigateToSafeSolved(page);
    await expect(page.getByText('CLASSIFIED DOSSIER')).toBeVisible({ timeout: 10000 });
    await page.getByRole('button', { name: /DISARM/i }).click();

    // Emails
    await expect(page.getByRole('link', { name: /morgie@agentmorgie\.com/i })).toBeVisible();
    await expect(page.getByRole('link', { name: /classified@agentmorgie\.com/i })).toBeVisible();
    await expect(page.getByRole('link', { name: /deepcover@agentmorgie\.com/i })).toBeVisible();
    await expect(page.getByRole('link', { name: /hq@agentmorgie\.com/i })).toBeVisible();
    ...
    await expect(page.getByRole('link', { name: /Amazon Wishlist/i })).toBeVisible();
  });
});
...
test.describe('Contact Page — Skip Flow', () => {
  test('skip shows dossier without puzzles or timer', async ({ page }) => {
    ...
    await expect(page.getByText('CLASSIFIED DOSSIER')).toBeVisible({ timeout: 5000 });
    await expect(page.getByText('morgie@agentmorgie.com').first()).toBeVisible();
    await expect(page.getByText(/SELF-DESTRUCT:/i)).not.toBeVisible();
    await expect(page.getByRole('button', { name: /TRY THE FULL MISSION/i })).toBeVisible();
  });
...
test.describe('Contact Page — Navigation & Reset', () => {
  ...
  test('TRY THE FULL MISSION from skip goes to entry', async ({ page }) => {
    await page.goto('/contact');
    await page.getByRole('button', { name: /Skip the Mission/i }).click();
    await expect(page.getByText('CLASSIFIED DOSSIER')).toBeVisible({ timeout: 5000 });
    await page.getByRole('button', { name: /TRY THE FULL MISSION/i }).click();
    await expect(page.getByText('OPERATION: CONTACT MORGIE')).toBeVisible({ timeout: 10000 });
  });
});
...
test.describe('Contact Page — SEO & Meta', () => {
  test('has correct page title', async ({ page }) => {
    await page.goto('/contact');
    await expect(page).toHaveTitle(/Operation: Contact Morgie/i);
  });

  test('has correct meta description', async ({ page }) => {
    await page.goto('/contact');
    const description = await page.locator('meta[name="description"]').getAttribute('content');
    expect(description).toContain('spy mission');
  });
...
async function navigateToCipherSolved(page: Page) {
  ...
  if (!box) throw new Error('Slider not found');
  await page.mouse.click(box.x + (box.width * 3 / 25), box.y + box.height / 2);
  await expect(page.getByText('PHASE 1 COMPLETE')).toBeVisible({ timeout: 3000 });
}
